...
object W3UResolver {
    ...
    suspend fun resolve(
        ...
        val resolved: ResolvedLink? = when {
            ...
            url.contains("bolaloca.my", true) ||
                    url.contains("cartelive.club", true) ||
                    url.contains("embedme.click", true) ->
                resolveGenericEmbed(url, referer, ua)

            source.embed ->
                resolveGenericEmbed(url, referer, ua)

            else ->
                resolveGenericEmbed(url, referer, ua) ?: ResolvedLink(url, referer ?: url)
        }

        if (resolved != null && (isDirectVideo(resolved.url) || !source.embed)) {
            addLink(
                name = "$channelName - S$sourceIndex",
                source = if (resolved.url == url) "W3U Directo $sourceIndex" else "W3U Resolver $sourceIndex",
                url = resolved.url,
