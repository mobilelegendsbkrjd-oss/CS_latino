...
open class DoodLaExtractor : ExtractorApi() {
    ...
    override suspend fun getUrl(
        ...
        val req = app.get(embedUrl, referer = referer ?: mainUrl)
        ...
        val md5 = host + passPath

        val token = md5.substringAfterLast("/")
        val direct = app.get(md5, referer = req.url).text

        if (direct.isBlank()) return

        val trueUrl = direct + createHashTable() + "?token=" + token
